...
public class Contiguous {
    ...
    SpaceManager checkSpace;

    public Contiguous(String dir, String path, BufferedReader f, List<String> data) throws FileNotFoundException {
        localDir = dir;
        filePath = path;
        file = f;
        fileData = data;
        ...
        checkSpace = new SpaceManager(fileData, endAllocationString, allocationString);
    ...
    public void createFile(String root, String sizeOfBlock) throws IOException {
        if (checkPath(root, 0)) {
            int onFileLocation = checkSpace.findFreeBlocks(sizeOfBlock);
            if (onFileLocation != -1) {
                String storedOnDisk = root + " " + Integer.toString(onFileLocation) + " " + sizeOfBlock;
                fileData.add(storedOnDisk);

                //New Edit
                int newAllocatedSize = abs(Integer.parseInt(fileData.get(endAllocationString + 2)) + Integer.parseInt(sizeOfBlock));
                int newFreeSize = abs(Integer.parseInt(fileData.get(endAllocationString + 3)) - Integer.parseInt(sizeOfBlock));

                //New Edit
                fileData.set(endAllocationString + 3, Integer.toString(newFreeSize));
                fileData.set(endAllocationString + 2, Integer.toString(newAllocatedSize));
                updateDisk();
                updateStructure();
            }
        }
    ...
    public void createFolder(String root) throws IOException {
        if (!(checkPath(root, 0))) {
            ...
            System.out.println("This folder already exists./ Wrong Path.");
        }
    }
    ...
    public void deleteFile(String root) throws IOException {
        ...
        if (!(checkPath(root, 1))) {
            ...
            checkSpace.deallocateBlocks(fileSearched);
        } else {
            System.out.println("No file exists");
            return;
        }

        int newAllocatedSize = abs(Integer.parseInt(fileData.get(4)) - Integer.parseInt(fileSearched.getSize()));
        int newFreeSize = abs(Integer.parseInt(fileData.get(5)) + Integer.parseInt(fileSearched.getSize()));
